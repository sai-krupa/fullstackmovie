  ...
  passport.authenticate('local', (err, user) => {
    ...
    req.logIn(user, (err) => {
      ...
      return res.json({
        ...
        token: token,
        id: req.user._id,
        watchMovies: req.user.watchMovies,
...
userRouter
  ...
  .post(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
    User.findById(req.user._id)
      .then((user) => {
        if (user) {
          user.watchMovies.push(req.body);
          user
            .save()
            .then((user) => {
              res.statusCode = 201;
              res.setHeader('Content-Type', 'application/json');
              res.json(user.watchMovies[user.watchMovies.length - 1]);
            })
            .catch((err) => next(err));
        } else {
          err = new Error(`User ${req.user._id} not found`);
  ...
  .put(cors.corsWithOptions, (req, res) => {
    res.statusCode = 403;
    res.end(`PUT operation not supported on /user/watchMovies`);
  })
  .delete(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
    User.findById(req.user._id)
      .then((user) => {
        if (user) {
          user.watchMovies = user.watchMovies.filter(
            (watchMovie) => watchMovie.movieId != req.body.movieId
          );
          user
            .save()
            .then((user) => {
